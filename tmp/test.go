...
func main() {
	...
	go func() {
		for i:=0; i<2000; i++ {
			memberid := GetMessageId("1",1);
			fmt.Println(memberid)
			fmt.Println("解码消息ID:",DecodeMessageID(memberid))
		}
	}()
...
// chatroom_id 会话ID
// sessionType 会话类型
func GetMessageId(chatroom_id string, sessionType int) (message_id string) {
	...
	sessionInt := (int64(crc32.ChecksumIEEE([]byte(sessionId)))) & 0x3FFFFF
...
func DecodeMessageID(message_id string) (BitId string) {
	for i := 0; i < 16; i++ {
