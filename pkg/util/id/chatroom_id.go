...
func (id *chatroomId) GetID(sessionType pbs.ChatroomType) (int64, error) {
	...
	max, err := rconn.Do("get", "id:chatroom_id:max")
	if err != nil {
		return 0, ErrTimeOut
	}
	var maxNum int64
	...
	//检查号池是否已存在
	exists, err := rconn.Do("exists", "id:chatroom_id:pond")
	if err != nil {
		return 0, ErrTimeOut
	}
	if exists.(int64) == 0 {
		addlen := 1000
		if _, err := rconn.Do("set", "id:chatroom_id:max", maxNum+int64(addlen)); err != nil {
			return 0, ErrTimeOut
		}
		for i := 0; i < addlen; i++ {
			maxNum++
			rconn.Do("sAdd", "id:chatroom_id:pond", maxNum)
		}
	}
	//随机抽取一个号
	cid, err := rconn.Do("sPop", "id:chatroom_id:pond")
	if err != nil {
		return 0, ErrTimeOut
	}
